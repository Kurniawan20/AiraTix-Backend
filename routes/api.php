...
use HiEvents\Http\Actions\Orders\GetAllTransactionsAction;
use HiEvents\Http\Actions\Orders\GetEventTransactionsAction;
...
use HiEvents\Http\Actions\Auth\Buyer\ForgotPasswordBuyerAction;
use HiEvents\Http\Actions\Auth\Buyer\GetBuyerOrderAction;
use HiEvents\Http\Actions\Auth\Buyer\GetBuyerOrdersAction;
use HiEvents\Http\Actions\Auth\Buyer\LoginBuyerAction;
use HiEvents\Http\Actions\Auth\Buyer\RegisterBuyerAction;
use HiEvents\Http\Actions\Auth\Buyer\ResetPasswordBuyerAction;
use HiEvents\Http\Actions\Auth\Buyer\ResendBuyerVerificationAction;
use HiEvents\Http\Actions\Auth\Buyer\VerifyBuyerEmailAction;
...
use HiEvents\Http\Actions\Auth\VerificationEmail;
...
use HiEvents\Http\Actions\Category\GetFormatAction;
use HiEvents\Http\Actions\Category\GetFormatWithPagination;
use HiEvents\Http\Actions\Category\GetTopicAction;
...
use HiEvents\Http\Actions\EventPublic\CreatePayment;
use HiEvents\Http\Actions\EventPublic\EventDetail;
...
use HiEvents\Http\Actions\Orders\HandleWebHookMidtrans;
...
use HiEvents\Http\Actions\Public\StoreEmailOrganizerAction;
...
$router->get('/public/get-format', GetFormatAction::class);
$router->get('/public/get-topic', GetTopicAction::class);
$router->get('/public/events-upcoming', EventPublicList::class);
$router->get('/public/event/{id}', EventDetail::class);
$router->get('/public/slides', EventHomePage::class);
$router->get('/public/event-online', EventOnline::class);
$router->get('/public/event-other', EventOther::class);
...
$router->prefix('/public')->group(
    function (Router $router): void {
        ...
        $router->post('/buyer/register', RegisterBuyerAction::class);
        $router->post('/buyer/login', LoginBuyerAction::class);
        $router->post('/buyer/forgot-password', ForgotPasswordBuyerAction::class)->name('buyer.password.request');
        $router->post('/buyer/reset-password', ResetPasswordBuyerAction::class)->name('buyer.password.reset');
        $router->get('/buyer/reset-password/{token}', function() {
            return response()->json(['message' => 'Token is valid']);
        })->name('password.reset');

        // Email Verification Routes
        $router->get('/buyer/email/verify/{id}/{hash}', VerifyBuyerEmailAction::class)
            ->middleware(['signed'])
            ->name('buyer.verification.verify');
        $router->post('/buyer/email/resend', ResendBuyerVerificationAction::class)
            ->middleware(['auth:buyer'])
            ->name('buyer.verification.resend');
        ...
        $router->get('/transactions', GetAllTransactionsAction::class);

        // Buyer Orders Routes
        $router->get('/buyer/orders', GetBuyerOrdersAction::class)
            ->middleware(['auth:buyer']);
        $router->get('/buyer/orders/{order_short_id}', GetBuyerOrderAction::class)
            ->middleware(['auth:buyer']);
        ...
        $router->post('/webhooks/midtrans', [HandleWebHookMidtrans::class, 'handleNotification']);
